...
import { User, ContentDTO, CategoryDTO } from '../common/types';
...
export const createCategory = async (req: Request, res: Response) => {
  const user: User = (<any>req).user;
  const categoryInfo: CategoryDTO = req.body;
  await categoryService.createCategory(user.id as number, categoryInfo);
